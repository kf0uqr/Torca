...
4. While tracking is running, **Split** (next to VFO A/B) turns on automatically -- **PTT** then swaps VFO B in with a Doppler-corrected *uplink* for the transmission and back to VFO A's downlink the instant you release, so RX and TX each track correctly through their own (opposite-direction) Doppler shift. Split turns back off when tracking is paused/stopped.
