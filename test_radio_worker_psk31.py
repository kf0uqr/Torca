"""
Wiring test for radio_worker.py's PSK31 decode attach/detach trio
(start_psk31_decode/stop_psk31_decode/_attach_psk31_decode/
_detach_psk31_decode) -- plumbing only, does NOT re-test psk31.py's own
DSP correctness (see test_psk31.py for that).
...
Run directly: ./bin/python3 test_radio_worker_psk31.py
"""
...
class FakeRadio:
    ...
    RadioWorker._attach_psk31_decode's direct-tap branch."""
...
class FakeBridge:
    ...
    _attach_psk31_decode/_detach_psk31_decode actually call."""
...
def test_direct_tap_path():
    ...
    worker = make_worker()
    received = []
    worker._psk31_decode_callback = lambda b: received.append(b)
    ...
    assert worker.radio.rx_callback == worker._on_psk31_decode_frame

    worker._on_psk31_decode_frame(b"\x01\x02\x03\x04")
...
def test_start_stop_psk31_decode_thread_safe_wrappers():
    """Exercises the actual public entry points (start_psk31_decode/
    stop_psk31_decode), which dispatch via
    ...
    print("--- start_psk31_decode/stop_psk31_decode (real running loop) ---")
    worker = make_worker()
    ...
    try:
        received = []
        worker.start_psk31_decode(lambda b: received.append(b))
        ...
        assert worker._psk31_decode_callback is not None

        worker._on_psk31_decode_frame(b"\x01\x02")
        assert wait_until(lambda: received == [b"\x01\x02"])

        worker.stop_psk31_decode()
        assert wait_until(lambda: worker.radio.stop_rx_calls == 1)
        assert wait_until(lambda: worker._psk31_decode_callback is None)
    finally:
...
def main():
    ...
    try:
        ...
        test_start_stop_psk31_decode_thread_safe_wrappers()
    except AssertionError as exc:
        print(f"FAILED: {exc}")
        ok = False
    if ok:
        print("ALL PSK31 WIRING TESTS PASSED")
