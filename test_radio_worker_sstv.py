"""
Wiring test for radio_worker.py's SSTV decode attach/detach trio
(start_sstv_decode/stop_sstv_decode/_attach_sstv_decode/
_detach_sstv_decode) -- plumbing only, does NOT re-test sstv.py's own
DSP correctness (see test_sstv.py for that).
...
Run directly: ./bin/python3 test_radio_worker_sstv.py
"""
...
class FakeRadio:
    ...
    RadioWorker._attach_sstv_decode's direct-tap branch."""
...
class FakeBridge:
    ...
    _attach_sstv_decode/_detach_sstv_decode actually call."""
...
def test_direct_tap_path():
    ...
    worker = make_worker()
    received = []
    worker._sstv_decode_callback = lambda b: received.append(b)

    asyncio.run(worker._attach_sstv_decode())
    assert worker._sstv_decode_via_bridge is False
    assert worker.radio.start_rx_calls == 1
    assert worker.radio.rx_callback == worker._on_sstv_decode_frame

    worker._on_sstv_decode_frame(b"\x01\x02\x03\x04")
    assert received == [b"\x01\x02\x03\x04"]

    asyncio.run(worker._detach_sstv_decode())
...
def test_bridge_piggyback_path():
    ...
    asyncio.run(worker._attach_sstv_decode())
    assert worker._sstv_decode_via_bridge is True
    assert worker.audio_bridge.extra_rx_callbacks == [worker._on_sstv_decode_frame_from_bridge]
    assert worker.radio.start_rx_calls == 0, "bridge path must not steal the direct tap"

    worker._on_sstv_decode_frame_from_bridge(b"\xaa\xbb")
...
def test_start_stop_sstv_decode_thread_safe_wrappers():
    """Exercises the actual public entry points (start_sstv_decode/
    stop_sstv_decode), which dispatch via
    ...
    print("--- start_sstv_decode/stop_sstv_decode (real running loop) ---")
    worker = make_worker()
    ...
    try:
        received = []
        worker.start_sstv_decode(lambda b: received.append(b))
        ...
        assert worker._sstv_decode_callback is not None

        worker._on_sstv_decode_frame(b"\x01\x02")
        assert wait_until(lambda: received == [b"\x01\x02"])

        worker.stop_sstv_decode()
        assert wait_until(lambda: worker.radio.stop_rx_calls == 1)
        assert wait_until(lambda: worker._sstv_decode_callback is None)
    finally:
...
def main():
    ...
    try:
        ...
        test_start_stop_sstv_decode_thread_safe_wrappers()
    except AssertionError as exc:
        print(f"FAILED: {exc}")
        ok = False
    if ok:
        print("ALL SSTV WIRING TESTS PASSED")
